function [thp,Tvp,rp] = fLRpar5(Te,pe,RH,z,dz)

%% Sfc parcel properties
    i0    = 1;
    T0    = Te(i0);
    RH0   = RH(i0);
    p0    = pe(i0);
...
pvstar = NaN(size(z));
pv = NaN(size(z));
gammaw(ilcl) = g*(1+(Lv*rp(ilcl))/(Rsd*Tp(ilcl)))/ ...
                (cpd+(Lv^2*rp(ilcl)*eps)/(Rsd*Tp(ilcl).^2));
...
for i=ilcl+1:length(z)
   ...
   gammaw(i) = g*(1+(Lv*rp(i))/(Rsd*Tp(i)))/(cpd+(Lv^2*rp(i)*eps)/(Rsd*Tp(i)^2));
